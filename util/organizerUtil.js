...
class OrganizerUtil {
  ...
  getById(organizerId, references) {
    ...
    const organizer = this._Organizer.query().where("ornizer_id", organizerId);
    return this._withReference(organizer, references)
      .fetch()
      .then((response) => response.first());
  }
  async create(organizerInstance, references) {
    const { organizer_id } = await this._Organizer.create(
      organizerInstance.body
    );
  ...
  async update(organizerInstance, references) {
    ...
    return this._withReference(organizers, references)
      .fetch()
      .then((response) => response.first());
  }
